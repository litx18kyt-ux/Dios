...
with sync_playwright() as p:
    ...
    # Hover over Sales & Targets to populate submenu in DOM
    page.evaluate('''() => {
    ...
    # Click on SPO Stockist Wise
    page.evaluate('''() => {
        const elms = Array.from(document.querySelectorAll('a, span, li'));
        const item = elms.find(e => (e.innerText || '').trim() === 'SPO Stockist Wise');
        if (item) item.click();
    }''')
    page.wait_for_timeout(2500)

    # Check for modal and set dates
    print(f"[3] Setting Dates in Modal: {from_date_str} to {to_date_str}...")
    page.evaluate('''(info) => {
    ...
    # 4. Scrape the rendered table directly from screen
    print("\n[4] Scraping SPO Stockist Table from Screen...")
    table_rows = []
    ...
    # 5. Click the green Excel icon on top-right
    print("\n[5] Downloading Excel file from Green Toolbar Icon...")
    try:
