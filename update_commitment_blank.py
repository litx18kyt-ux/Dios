...
export const CommitmentSheet: React.FC = () => {
  ...
  // Start with a completely blank table as requested ("sare doctor ko khali kar do")
  ...
  const handleAddDoctor = () => {
    setDoctorsRows(prev => {
      ...
      const updated = [
        ...prev,
        { sn: newSn, hq: 'UDAIPUR', drName: '', typeOfSupport: '', amount: '', expectedRoi: '', commitments: {}, achievements: {} }
      ];
      memoryStore.commitmentDoctors = updated;
      return updated;
    });
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl space-y-6">
      ...
      {/* SECTION 1: COMMITMENT OF MONTH (TOP BOX) */}
      ...
      {/* SECTION 2: SUPPORT REQUIREMENT & MONTHLY COMMITMENT/ACHIEVEMENT (NICHE BOX) */}
      <div className="space-y-4 bg-slate-950 p-4 rounded-2xl border border-slate-800">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-800">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider">Support Requirement &amp; Monthly Commitments (CSV Format)</h3>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {doctorsRows.length === 0 ? (
                <tr>
                  <td colSpan={7 + MONTH_CODES.length * 2} className="p-8 text-center text-slate-500">
                    No support rows added yet. Click <b>"Add Support Row"</b> above to enter doctor support and monthly commitments.
                  </td>
                </tr>
              ) : (
                doctorsRows.map(doc => (
                      ...
                      <React.Fragment key={m}>
                        ...
                        <td className="p-0.5 text-center bg-blue-950/20">
                          <input
                            ...
                            placeholder="C"
                            className="w-12 py-1 px-1 bg-transparent text-center font-mono text-[11px] text-blue-300 focus:bg-slate-900 focus:outline-none"
                          />
                        </td>
                        {/* Achievement */}
                        <td className="p-0.5 text-center border-r border-slate-800/60 bg-emerald-950/20">
                          <input
                            ...
                            placeholder="A"
                            className="w-12 py-1 px-1 bg-transparent text-center font-mono text-[11px] text-emerald-300 font-bold focus:bg-slate-900 focus:outline-none"
                          />
                        </td>
                      </React.Fragment>
                    ))}
...
print('✅ CommitmentSheet.tsx updated with blank support table & positioned correctly at bottom!')
