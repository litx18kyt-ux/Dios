  ...
  if (isPdf) {
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
      ...
      lineBins.forEach(bin => {
        ...
        if (!fullLine || fullLine.toUpperCase().includes('NAGDA') || fullLine.toUpperCase().includes('STATEMENT') || fullLine.toUpperCase().includes('PRODUCT DESCRIPTION') || fullLine.toUpperCase().includes('TOTAL')) return;
        ...
        for (let i = tokens.length - 1; i >= 0; i--) {
          ...
          if (/^-?\d+(\.\d+)?$/.test(t) && nums.length < 11) {
            nums.unshift(parseFloat(t));
          } else {
            nameTokens.unshift(tokens[i]);
          }
        }

        if (nameTokens.length > 0 && nums.length >= 4) {
          ...
          if (matched) {
            ...
            // Format B (10-11 Numbers: Op, Pur, Return, Repl, TotRec, SaleQty(5), PR, Repl, Closing(8), Order)
            if (nums.length >= 10) {
              sales = nums[5];
              closing = nums[8];
            }
            // Format A (8 Numbers: Op, OpVal, Rec, RecVal, IssueQty(4), IssueVal, ClosingStock(6), ClosingVal)
    ...
    // 2D Array Engine for XLS / XLSX / CSV
    const arrayBuffer = await file.arrayBuffer();
    ...
    // Strict Header Detection (MUST contain both PRODUCT and OPENING/STOCK - skips title row!)
    for (let r = 0; r < Math.min(rows.length, 12); r++) {
      const row = rows[r] || [];
      const rowStr = row.map(c => String(c || '').toUpperCase().trim()).join(' | ');

      // Ignore title lines like "STOCK & SALES STATEMENT"
      if (rowStr.includes('PRODUCT') && (rowStr.includes('OPENING') || rowStr.includes('PURCHASE') || rowStr.includes('RECEIVE') || rowStr.includes('RATE') || rowStr.includes('ORDER'))) {
        startRow = r + 1;
        row.forEach((cell, cIdx) => {
          const c = String(cell || '').replace(/[\r\n]+/g, ' ').toUpperCase().trim();
          if (c.includes('PRODUCT') || c.includes('DESCRIPTION')) {
            prodCol = cIdx;
          }
          // Sales: strictly match SALE QUANTITY or ISSUE QUANTITY (not RETURN, not P/R, not VALUE, not RECEIVE)
          if ((c === 'SALE QUANTITY' || c === 'SALE QTY' || c === 'ISSUE QUANTITY' || c === 'ISSUE QTY') || 
              ((c.includes('SALE') || c.includes('ISSUE')) && c.includes('QTY') && !c.includes('RETURN') && !c.includes('P/R') && !c.includes('VAL') && !c.includes('RECEIVE'))) {
            salesCol = cIdx;
          }
          // Closing: strictly match CLOSING STOCK or CLOSING BALANCE (not VALUE)
