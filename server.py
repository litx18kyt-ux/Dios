...
app = FastAPI(title="DIOS CBO Master Unified API")
...
@app.get("/")
def root():
    return {"status": "online", "engine": "DIOS Master Unified Clean Engine", "version": "v61.0"}

# -----------------------------------------------------------------------------
# 1. SALES PERFORMANCE LIVE BOT (SPO STOCKIST-WISE)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 2. MONTH FW PROGRESS LIVE BOT (DCR DATE-WISE EXCEL)
# -----------------------------------------------------------------------------
        ...
        try:
            ...
            with page.expect_download(timeout=15000) as dl_info:
                page.evaluate('''() => {
                    ...
                    const btn = elms.find(e => (e.className || '').includes('excel') || (e.title || '').includes('excel') || (e.className || '').includes('fa-file-excel'));
                    if (btn) btn.click();
                }''')
            dl = dl_info.value
            ...
            return Response(content=content, media_type="application/vnd.ms-excel", headers={"Content-Disposition": f"inline; filename=DCR_DateWise_{req.from_month}.xls"})
        except Exception as e:
            browser.close()
            raise HTTPException(status_code=500, detail=str(e))

# -----------------------------------------------------------------------------
# 3. PRIMARY SALES LIVE SCRAPER (FOR STATEMENT AGGREGATOR)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 4. PRIMARY CBO EXCEL DOWNLOAD ENDPOINT
# -----------------------------------------------------------------------------
...
def fetch_cbo_excel(req: MonthRequest):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            return Response(content=content, media_type="application/vnd.ms-excel", headers={"Content-Disposition": f"inline; filename=CBO_Primary_{req.from_month}.xls"})
        except Exception as e:
            browser.close()
            raise HTTPException(status_code=500, detail=str(e))
