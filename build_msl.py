        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead className="sticky top-0 bg-slate-950 text-slate-400 font-bold uppercase border-b border-slate-800 z-30">
            <tr>
              ...
              <th className="p-2.5 text-center w-24 bg-slate-950 border-r-4 border-cyan-400 shadow-2xl sticky left-[634px] z-40 bg-slate-950">DOA</th>
              <th className="p-2.5 text-center min-w-[140px] text-cyan-400 bg-slate-950">APR</th>
              <th className="p-2.5 text-center min-w-[140px] text-emerald-400 bg-slate-950">MAY</th>
              <th className="p-2.5 text-center min-w-[140px] text-purple-400 bg-slate-950">JUN</th>
              <th className="p-2.5 text-center min-w-[140px] text-blue-400 bg-slate-950">JUL</th>
              <th className="p-2.5 text-center min-w-[140px] text-amber-400 bg-slate-950">AUG</th>
              <th className="p-2.5 text-center min-w-[140px] text-rose-400 bg-slate-950">SEPT</th>
              <th className="p-2.5 text-center min-w-[140px] text-cyan-300 bg-slate-950">OCT</th>
              <th className="p-2.5 text-center min-w-[140px] text-emerald-300 bg-slate-950">NOV</th>
              <th className="p-2.5 text-center min-w-[140px] text-purple-300 bg-slate-950">DEC</th>
              <th className="p-2.5 text-center min-w-[140px] text-blue-300 bg-slate-950">JAN</th>
              <th className="p-2.5 text-center min-w-[140px] text-amber-300 bg-slate-950">FEB</th>
              <th className="p-2.5 text-center min-w-[140px] text-rose-300 bg-slate-950">MAR</th>
          ...
          <tbody className="divide-y divide-slate-800/60">
            {{filtered.map(doc => (
              <tr key={{doc.srNo}} className="hover:bg-slate-800/40 transition">
                ...
                <td className="p-1 border-r-4 border-cyan-400 shadow-2xl sticky left-[634px] bg-slate-900 z-20 w-24">
                  <input
                    type="text"
                    value={{doc.doa}}
                ...
                {{['apr', 'may', 'jun', 'jul', 'aug', 'sept', 'oct', 'nov', 'dec', 'jan', 'feb', 'mar'].map(monthKey => (
                  <td key={{monthKey}} className="p-1.5 min-w-[140px]">
                    <input
                      type="text"
                      value={{(doc as any)[monthKey]}}
                      onChange={{e => handleFieldChange(doc.srNo, monthKey as any, e.target.value)}}
                      placeholder="-"
                      className="w-full py-2 px-2.5 bg-slate-950 rounded-xl font-mono text-slate-100 border border-slate-700 focus:border-cyan-400 focus:outline-none text-center text-xs font-bold"
                    />
                  </td>
                ))}}
...
with open('src/components/review/MslSheet.tsx', 'w') as out_f:
    out_f.write(ts_content)
print("✅ MslSheet.tsx generated successfully!")
''')

import subprocess
subprocess.run(['python3', 'build_msl.py'])
subprocess.run(['npm', 'run', 'build'])
subprocess.run(['npx', 'wrangler', 'pages', 'deploy', 'dist', '--project-name=dios-hub', '--branch=main', '--commit-dirty=true'])
